...
const Zona = (props) => {
  ...
  const saveZona = () => {
    if (editing) {
      Api.service("zona").patch(dato.id, { nombre }).then(() => { getZonas() });
    } else {
      Api.service("zona").create({ nombre }).then(() => { getZonas() });
    }
    limpiar()
  };
                ...
                datos.data.map((dato, num) => {
                  return (
                    <tr key={dato.id}>
                      ...
                      <td>{dato.nombre}</td>
                      <td>
                        <Button variant="link" onClick={() => editarZona(dato)}>Editar</Button>
                      </td>
                    </tr>
                  );
                })}
